<template>
  <view class="l-f2" :style="customStyle" v-if="canvasId">
    ...
    <view
      v-if="isCloud"
      style="width: 2048px; height: 2048px; position: fixed; left: 9999px"
    >
      <canvas
        v-if="use2dCanvas"
        type="2d"
        :canvas-id="canvasId + '_cloud'"
        :id="canvasId + '_cloud'"
        class="l-f2__canvas"
      ></canvas>
      <canvas
    ...
    <web-view
      ...
      src="/uni_modules/lime-f2/static/index.html"
...
export default {
  ...
  created() {
    ...
    // #ifdef MP-ALIPAY
    this.use2dCanvas =
      this.type === "2d" && compareVersion(my.SDKVersion, "2.7.15") >= 0;
    // #endif
  },
  // async mounted() {
  // 	if (this.onInit) {
  ...
  methods: {
    ...
    async init(func, params = null) {
        ...
        (n, o) => {
          (n || o) &&
            ...
            this.$refs.webview.evalJs(
              `init(${JSON.stringify(func.toString())}, ${JSON.stringify(
                params
              )})`
            );
          this.isInited = true;
      ...
      if (this.isCloud) {
        ...
        const cloud = await this.getContext(this.canvasId + "_cloud");
    ...
    changeData(data) {
      ...
      // #ifdef APP-NVUE
      this.$refs.webview.evalJs(
        `changeData(${JSON.stringify(data || this.source)})`
      );
      // #endif
    },
    clear() {
    ...
    repaint() {
      this.changeData(this.sourceData);
    },
    reset(func, params = null) {
      ...
      // #ifdef APP-NVUE
      this.$refs.webview.evalJs(
        `reset(${JSON.stringify(func.toString())}, ${JSON.stringify(
          params || this.params
        )})`
      );
      // #endif
        ...
        uni.getImageInfo({
          ...
          success: async (res) => {
            if (res.path) {
              ...
              const localReg = /^\.|^\/(?=[^\/])/;
              res.path = localReg.test(this.imageMask)
                ? `/${res.path}`
                : res.path;
              // #endif
        ...
        return new Promise((resolve) => {
          uni
            ...
            .exec((res) => {
              ...
              context.uniTransform = context.transform;
              context.transform = (...agrs) => {
                if (agrs.length > 6) {
                  agrs.pop();
                }
                context.uniTransform(...agrs);
              };
